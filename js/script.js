...
(function () {
	...
	$(function () {
		...
		// RD Input Label
		// Guard: avoid crash if RDInputLabel plugin is missing on some pages
		if (plugins.rdInputLabel && plugins.rdInputLabel.length && $.fn.RDInputLabel) {
		 ...
		 */
		// Select2
		// Guard: avoid crash if Select2 library is not included on this page
		if (plugins.selectFilter.length && $.fn.select2) {
			var i;
			if (!plugins.selectFilter || !plugins.selectFilter.length) return;
